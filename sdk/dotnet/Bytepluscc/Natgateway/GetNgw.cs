// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Byteplus.Pulumi.Bytepluscc.Natgateway
{
    public static class GetNgw
    {
        /// <summary>
        /// Data Source schema for Byteplus::NatGateway::NGW
        /// </summary>
        public static Task<GetNgwResult> InvokeAsync(GetNgwArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.InvokeAsync<GetNgwResult>("bytepluscc:natgateway/getNgw:getNgw", args ?? new GetNgwArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::NatGateway::NGW
        /// </summary>
        public static Output<GetNgwResult> Invoke(GetNgwInvokeArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.Invoke<GetNgwResult>("bytepluscc:natgateway/getNgw:getNgw", args ?? new GetNgwInvokeArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::NatGateway::NGW
        /// </summary>
        public static Output<GetNgwResult> Invoke(GetNgwInvokeArgs args, InvokeOutputOptions options)
            => global::Pulumi.Deployment.Instance.Invoke<GetNgwResult>("bytepluscc:natgateway/getNgw:getNgw", args ?? new GetNgwInvokeArgs(), options.WithDefaults());
    }


    public sealed class GetNgwArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public string Id { get; set; } = null!;

        public GetNgwArgs()
        {
        }
        public static new GetNgwArgs Empty => new GetNgwArgs();
    }

    public sealed class GetNgwInvokeArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public Input<string> Id { get; set; } = null!;

        public GetNgwInvokeArgs()
        {
        }
        public static new GetNgwInvokeArgs Empty => new GetNgwInvokeArgs();
    }


    [OutputType]
    public sealed class GetNgwResult
    {
        /// <summary>
        /// NAT网关的计费方式。取值如下：1 ：包年包月。2（默认值）：按量计费-按规格计费。3：按量计费-按使用量计费。当NetworkType传入intranet时，本参数仅支持且必须传入3。
        /// </summary>
        public readonly int BillingType;
        /// <summary>
        /// NAT网关是否被锁定。空值：正常未被锁定。Normal：正常未被锁定。FinancialLocked：因欠费被锁定。
        /// </summary>
        public readonly string BusinessStatus;
        /// <summary>
        /// 创建时间
        /// </summary>
        public readonly string CreatedTime;
        /// <summary>
        /// 删除时间
        /// </summary>
        public readonly string DeletedTime;
        /// <summary>
        /// NAT网关的描述。需要以字母、中文或数字开头。可包含英文逗号（,）、点号（.）、下划线（_）、空格（ ）、等号（=）、短横线（-）、中文逗号（，）、中文句号（。）。长度限制为1 ～ 255个字符。不填默认空字符串。
        /// </summary>
        public readonly string Description;
        /// <summary>
        /// 标识该nat是直通还是非直通。   - true（默认）：EIP直通nat网关。   - false：非EIP直通nat网关。
        /// </summary>
        public readonly bool DirectMode;
        /// <summary>
        /// DNAT规则ID列表。
        /// </summary>
        public readonly ImmutableArray<string> DnatEntryIds;
        /// <summary>
        /// NAT网关绑定公网IP的信息。
        /// </summary>
        public readonly ImmutableArray<Outputs.GetNgwEipAddressResult> EipAddresses;
        /// <summary>
        /// 包年包月NAT网关到期时间。仅包年包月计费类型的NAT网关会返回此参数。
        /// </summary>
        public readonly string ExpiredTime;
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        public readonly string Id;
        /// <summary>
        /// NAT被锁定的原因。financial：因欠费被锁定。security：因安全原因被锁定。
        /// </summary>
        public readonly string LockReason;
        /// <summary>
        /// NAT网关ID。
        /// </summary>
        public readonly string NatGatewayId;
        /// <summary>
        /// NAT网关的名称。需要以字母、中文或数字开头，可包含点号（.）、下划线（_）和短划线（-）。长度限制为1 ~ 128个字符。不填默认是NAT网关实例的ID。
        /// </summary>
        public readonly string NatGatewayName;
        /// <summary>
        /// 私网NAT实例的中转IP列表。
        /// </summary>
        public readonly ImmutableArray<Outputs.GetNgwNatIpAddressResult> NatIpAddresses;
        /// <summary>
        /// NAT网关占用的网卡ID。
        /// </summary>
        public readonly string NetworkInterfaceId;
        /// <summary>
        /// NAT网关类型。internet：公网NAT网关。intranet：私网NAT网关。
        /// </summary>
        public readonly string NetworkType;
        /// <summary>
        /// 资源冻结时间。仅当资源因为欠费冻结，此参数才会有返回值。
        /// </summary>
        public readonly string OverdueTime;
        /// <summary>
        /// 购买包年包月NAT网关的时长，默认1。当PeriodUnit传入Month，Period取值范围：1 ~ 9、12、24、36。当PeriodUnit传入Year，Period取值范围：1 ~ 3。
        /// </summary>
        public readonly int Period;
        /// <summary>
        /// 购买包年包月NAT网关时长的单位，默认Month。Month ：月。Year ：年。
        /// </summary>
        public readonly string PeriodUnit;
        /// <summary>
        /// NAT网关所属项目的名称。不填默认加入default项目。
        /// </summary>
        public readonly string ProjectName;
        /// <summary>
        /// 是否开启智能调度。
        /// </summary>
        public readonly bool SmartScheduleEnabled;
        /// <summary>
        /// 智能调度策略规则。ChinaMobile：中国移动。ChinaUnicom：中国联通。ChinaTelecom：中国电信。BGP：BGP多线。
        /// </summary>
        public readonly string SmartScheduleRule;
        /// <summary>
        /// SNAT规则ID列表。
        /// </summary>
        public readonly ImmutableArray<string> SnatEntryIds;
        /// <summary>
        /// NAT网关的规格。Small（默认）：小型。Medium：中型。Large：大型。Extra*Large*1：超大型-1。Extra*Large*2：超大型-2。
        /// </summary>
        public readonly string Spec;
        /// <summary>
        /// NAT网关的状态。Available：可用。Creating：创建中。Pending：操作中。Deleting：删除中。
        /// </summary>
        public readonly string Status;
        /// <summary>
        /// NAT网关所在子网的ID。传入的子网，必须是VpcId传入的私有网络的子网。子网内至少有一个可用的私网IP。系统自动生成的NAT网关网卡将占用该子网一个私网IP。子网所在的可用区需要已部署NAT网关资源。子网所在可用区将作为NAT网关的主可用区，并自动关联备可用区。当主可用区发生故障时，NAT网关自动切换到备可用区。
        /// </summary>
        public readonly string SubnetId;
        /// <summary>
        /// 标签列表。
        /// </summary>
        public readonly ImmutableArray<Outputs.GetNgwTagResult> Tags;
        /// <summary>
        /// NAT网关的最后操作时间。
        /// </summary>
        public readonly string UpdatedTime;
        /// <summary>
        /// NAT网关所在私有网络的ID。
        /// </summary>
        public readonly string VpcId;
        /// <summary>
        /// NAT网关所属主可用区的ID。
        /// </summary>
        public readonly string ZoneId;

        [OutputConstructor]
        private GetNgwResult(
            int billingType,

            string businessStatus,

            string createdTime,

            string deletedTime,

            string description,

            bool directMode,

            ImmutableArray<string> dnatEntryIds,

            ImmutableArray<Outputs.GetNgwEipAddressResult> eipAddresses,

            string expiredTime,

            string id,

            string lockReason,

            string natGatewayId,

            string natGatewayName,

            ImmutableArray<Outputs.GetNgwNatIpAddressResult> natIpAddresses,

            string networkInterfaceId,

            string networkType,

            string overdueTime,

            int period,

            string periodUnit,

            string projectName,

            bool smartScheduleEnabled,

            string smartScheduleRule,

            ImmutableArray<string> snatEntryIds,

            string spec,

            string status,

            string subnetId,

            ImmutableArray<Outputs.GetNgwTagResult> tags,

            string updatedTime,

            string vpcId,

            string zoneId)
        {
            BillingType = billingType;
            BusinessStatus = businessStatus;
            CreatedTime = createdTime;
            DeletedTime = deletedTime;
            Description = description;
            DirectMode = directMode;
            DnatEntryIds = dnatEntryIds;
            EipAddresses = eipAddresses;
            ExpiredTime = expiredTime;
            Id = id;
            LockReason = lockReason;
            NatGatewayId = natGatewayId;
            NatGatewayName = natGatewayName;
            NatIpAddresses = natIpAddresses;
            NetworkInterfaceId = networkInterfaceId;
            NetworkType = networkType;
            OverdueTime = overdueTime;
            Period = period;
            PeriodUnit = periodUnit;
            ProjectName = projectName;
            SmartScheduleEnabled = smartScheduleEnabled;
            SmartScheduleRule = smartScheduleRule;
            SnatEntryIds = snatEntryIds;
            Spec = spec;
            Status = status;
            SubnetId = subnetId;
            Tags = tags;
            UpdatedTime = updatedTime;
            VpcId = vpcId;
            ZoneId = zoneId;
        }
    }
}
