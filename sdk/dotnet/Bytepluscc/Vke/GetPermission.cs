// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Byteplus.Pulumi.Bytepluscc.Vke
{
    public static class GetPermission
    {
        /// <summary>
        /// Data Source schema for Byteplus::VKE::Permission
        /// </summary>
        public static Task<GetPermissionResult> InvokeAsync(GetPermissionArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.InvokeAsync<GetPermissionResult>("bytepluscc:vke/getPermission:getPermission", args ?? new GetPermissionArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::VKE::Permission
        /// </summary>
        public static Output<GetPermissionResult> Invoke(GetPermissionInvokeArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.Invoke<GetPermissionResult>("bytepluscc:vke/getPermission:getPermission", args ?? new GetPermissionInvokeArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::VKE::Permission
        /// </summary>
        public static Output<GetPermissionResult> Invoke(GetPermissionInvokeArgs args, InvokeOutputOptions options)
            => global::Pulumi.Deployment.Instance.Invoke<GetPermissionResult>("bytepluscc:vke/getPermission:getPermission", args ?? new GetPermissionInvokeArgs(), options.WithDefaults());
    }


    public sealed class GetPermissionArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public string Id { get; set; } = null!;

        public GetPermissionArgs()
        {
        }
        public static new GetPermissionArgs Empty => new GetPermissionArgs();
    }

    public sealed class GetPermissionInvokeArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public Input<string> Id { get; set; } = null!;

        public GetPermissionInvokeArgs()
        {
        }
        public static new GetPermissionInvokeArgs Empty => new GetPermissionInvokeArgs();
    }


    [OutputType]
    public sealed class GetPermissionResult
    {
        /// <summary>
        /// RBAC 权限策略资源被授权的时间。
        /// </summary>
        public readonly string AuthorizedAt;
        /// <summary>
        /// 授权者的 ID。可以是 IAM 用户 ID，也可以是 IAM 角色 ID。
        /// </summary>
        public readonly int AuthorizerId;
        /// <summary>
        /// 授权者名称
        /// </summary>
        public readonly string AuthorizerName;
        /// <summary>
        /// 授权者的类型，取值如下：User：IAM 用户。Role：IAM 角色。Account：账号。
        /// </summary>
        public readonly string AuthorizerType;
        /// <summary>
        /// 需要授权给 IAM 用户或角色的集群 ID。roleDomain为namespace或cluster时必须填写该参数值。
        /// </summary>
        public readonly string ClusterId;
        /// <summary>
        /// RBAC 权限策略资源创建的时间。
        /// </summary>
        public readonly string CreatedTime;
        /// <summary>
        /// 被授权者的 ID。可以是 IAM 用户 ID，也可以是 IAM 角色 ID。
        /// </summary>
        public readonly int GranteeId;
        /// <summary>
        /// 被授权者的类型，取值如下：User：IAM 用户。Role：IAM 角色。Account：账号。
        /// </summary>
        public readonly string GranteeType;
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        public readonly string Id;
        /// <summary>
        /// 给被授权者授予的 RBAC 角色是否为自定义角色，取值如下：true：自定义角色。false：（默认值）系统预置的角色。
        /// </summary>
        public readonly bool IsCustomRole;
        /// <summary>
        /// RBAC 权限策略资源在 Kubernetes 中的对象名称。
        /// </summary>
        public readonly string KubeRoleBindingName;
        /// <summary>
        /// 授权详细消息。
        /// </summary>
        public readonly string Message;
        /// <summary>
        /// 需要授权给 IAM 用户或角色的名空间名称。roleDomain等于namespace时必须填写该参数值。roleDomain不等于namespace时，填写该参数值不生效。
        /// </summary>
        public readonly string Namespace;
        /// <summary>
        /// RBAC 权限策略资源 ID。
        /// </summary>
        public readonly string PermissionId;
        /// <summary>
        /// 项目选择器
        /// </summary>
        public readonly string ProjectSelector;
        /// <summary>
        /// 撤销授权的时间。
        /// </summary>
        public readonly string RevokedAt;
        /// <summary>
        /// 为 IAM 用户或角色授予的权限类型，取值如下：namespace：授予命名空间级别的权限。cluster：授予集群级别的权限。all_clusters：授予当前账号下全部集群级别的权限。
        /// </summary>
        public readonly string RoleDomain;
        /// <summary>
        /// 给被授权者授予的 RBAC 角色名称。目前必须填写该参数。当roleDomain为all_clusters时，不可以自定义角色名称。
        /// </summary>
        public readonly string RoleName;
        /// <summary>
        /// RBAC 权限访问策略资源的状态，取值如下：Success：授权成功。Failed：授权失败。Pending：授权中。PartialSuccess：部分授权成功。
        /// </summary>
        public readonly string Status;

        [OutputConstructor]
        private GetPermissionResult(
            string authorizedAt,

            int authorizerId,

            string authorizerName,

            string authorizerType,

            string clusterId,

            string createdTime,

            int granteeId,

            string granteeType,

            string id,

            bool isCustomRole,

            string kubeRoleBindingName,

            string message,

            string @namespace,

            string permissionId,

            string projectSelector,

            string revokedAt,

            string roleDomain,

            string roleName,

            string status)
        {
            AuthorizedAt = authorizedAt;
            AuthorizerId = authorizerId;
            AuthorizerName = authorizerName;
            AuthorizerType = authorizerType;
            ClusterId = clusterId;
            CreatedTime = createdTime;
            GranteeId = granteeId;
            GranteeType = granteeType;
            Id = id;
            IsCustomRole = isCustomRole;
            KubeRoleBindingName = kubeRoleBindingName;
            Message = message;
            Namespace = @namespace;
            PermissionId = permissionId;
            ProjectSelector = projectSelector;
            RevokedAt = revokedAt;
            RoleDomain = roleDomain;
            RoleName = roleName;
            Status = status;
        }
    }
}
