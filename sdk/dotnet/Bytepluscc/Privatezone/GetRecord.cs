// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Byteplus.Pulumi.Bytepluscc.Privatezone
{
    public static class GetRecord
    {
        /// <summary>
        /// Data Source schema for Byteplus::PrivateZone::Record
        /// </summary>
        public static Task<GetRecordResult> InvokeAsync(GetRecordArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.InvokeAsync<GetRecordResult>("bytepluscc:privatezone/getRecord:getRecord", args ?? new GetRecordArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::PrivateZone::Record
        /// </summary>
        public static Output<GetRecordResult> Invoke(GetRecordInvokeArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.Invoke<GetRecordResult>("bytepluscc:privatezone/getRecord:getRecord", args ?? new GetRecordInvokeArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::PrivateZone::Record
        /// </summary>
        public static Output<GetRecordResult> Invoke(GetRecordInvokeArgs args, InvokeOutputOptions options)
            => global::Pulumi.Deployment.Instance.Invoke<GetRecordResult>("bytepluscc:privatezone/getRecord:getRecord", args ?? new GetRecordInvokeArgs(), options.WithDefaults());
    }


    public sealed class GetRecordArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public string Id { get; set; } = null!;

        public GetRecordArgs()
        {
        }
        public static new GetRecordArgs Empty => new GetRecordArgs();
    }

    public sealed class GetRecordInvokeArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public Input<string> Id { get; set; } = null!;

        public GetRecordInvokeArgs()
        {
        }
        public static new GetRecordInvokeArgs Empty => new GetRecordInvokeArgs();
    }


    [OutputType]
    public sealed class GetRecordResult
    {
        /// <summary>
        /// Creation time of the DNS record.
        /// </summary>
        public readonly string CreatedAt;
        /// <summary>
        /// Indicates whether the DNS record is enabled.
        /// </summary>
        public readonly bool Enable;
        /// <summary>
        /// Domain prefix for the subdomain.
        /// </summary>
        public readonly string Host;
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        public readonly string Id;
        /// <summary>
        /// Account ID of the most recent update to the DNS record.
        /// </summary>
        public readonly string LastOperator;
        /// <summary>
        /// DNS record route, only the default route 'default' is supported.
        /// </summary>
        public readonly string Line;
        /// <summary>
        /// DNS record ID.
        /// </summary>
        public readonly string RecordId;
        /// <summary>
        /// Remarks. Supports UTF-8 characters. Maximum 16 characters. Default is an empty string.
        /// </summary>
        public readonly string Remark;
        /// <summary>
        /// DNS record TTL (time to live), in seconds. Default is 600.
        /// </summary>
        public readonly int Ttl;
        /// <summary>
        /// DNS record type. Valid values are: A: A record. AAAA: AAAA record. CNAME: CNAME record. TXT: TXT record. MX: MX record. PTR: PTR record. A: A record. AAAA: AAAA record. CNAME: CNAME record. TXT: TXT record. MX: MX record. PTR: PTR record.
        /// </summary>
        public readonly string Type;
        /// <summary>
        /// Last update time of the DNS record.
        /// </summary>
        public readonly string UpdatedAt;
        /// <summary>
        /// Record value.
        /// </summary>
        public readonly string Value;
        /// <summary>
        /// Record weight. The weight takes effect only when load balancing is enabled for the domain name.
        /// </summary>
        public readonly int Weight;
        /// <summary>
        /// Indicates whether load balancing is enabled for this record set.
        /// </summary>
        public readonly bool WeightEnabled;
        /// <summary>
        /// Domain name ID.
        /// </summary>
        public readonly int Zid;

        [OutputConstructor]
        private GetRecordResult(
            string createdAt,

            bool enable,

            string host,

            string id,

            string lastOperator,

            string line,

            string recordId,

            string remark,

            int ttl,

            string type,

            string updatedAt,

            string value,

            int weight,

            bool weightEnabled,

            int zid)
        {
            CreatedAt = createdAt;
            Enable = enable;
            Host = host;
            Id = id;
            LastOperator = lastOperator;
            Line = line;
            RecordId = recordId;
            Remark = remark;
            Ttl = ttl;
            Type = type;
            UpdatedAt = updatedAt;
            Value = value;
            Weight = weight;
            WeightEnabled = weightEnabled;
            Zid = zid;
        }
    }
}
