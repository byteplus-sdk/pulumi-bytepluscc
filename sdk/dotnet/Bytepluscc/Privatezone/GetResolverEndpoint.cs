// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Byteplus.Pulumi.Bytepluscc.Privatezone
{
    public static class GetResolverEndpoint
    {
        /// <summary>
        /// Data Source schema for Byteplus::PrivateZone::ResolverEndpoint
        /// </summary>
        public static Task<GetResolverEndpointResult> InvokeAsync(GetResolverEndpointArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.InvokeAsync<GetResolverEndpointResult>("bytepluscc:privatezone/getResolverEndpoint:getResolverEndpoint", args ?? new GetResolverEndpointArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::PrivateZone::ResolverEndpoint
        /// </summary>
        public static Output<GetResolverEndpointResult> Invoke(GetResolverEndpointInvokeArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.Invoke<GetResolverEndpointResult>("bytepluscc:privatezone/getResolverEndpoint:getResolverEndpoint", args ?? new GetResolverEndpointInvokeArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::PrivateZone::ResolverEndpoint
        /// </summary>
        public static Output<GetResolverEndpointResult> Invoke(GetResolverEndpointInvokeArgs args, InvokeOutputOptions options)
            => global::Pulumi.Deployment.Instance.Invoke<GetResolverEndpointResult>("bytepluscc:privatezone/getResolverEndpoint:getResolverEndpoint", args ?? new GetResolverEndpointInvokeArgs(), options.WithDefaults());
    }


    public sealed class GetResolverEndpointArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public string Id { get; set; } = null!;

        public GetResolverEndpointArgs()
        {
        }
        public static new GetResolverEndpointArgs Empty => new GetResolverEndpointArgs();
    }

    public sealed class GetResolverEndpointInvokeArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public Input<string> Id { get; set; } = null!;

        public GetResolverEndpointInvokeArgs()
        {
        }
        public static new GetResolverEndpointInvokeArgs Empty => new GetResolverEndpointInvokeArgs();
    }


    [OutputType]
    public sealed class GetResolverEndpointResult
    {
        /// <summary>
        /// Creation time
        /// </summary>
        public readonly string CreatedTime;
        /// <summary>
        /// DNS request forwarding direction for the endpoint. OUTBOUND (default): outbound endpoint forwards DNS queries from within the VPC to external DNS servers. INBOUND: inbound endpoint forwards DNS queries from external sources to the resolver
        /// </summary>
        public readonly string Direction;
        /// <summary>
        /// Endpoint ID
        /// </summary>
        public readonly string EndpointId;
        /// <summary>
        /// Endpoint type. IPv4: IPv4 endpoint. DualStack: dual-stack endpoint
        /// </summary>
        public readonly string EndpointType;
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        public readonly string Id;
        /// <summary>
        /// Availability zone, subnet, and IP configuration for the endpoint
        /// </summary>
        public readonly ImmutableArray<Outputs.GetResolverEndpointIpConfigResult> IpConfigs;
        /// <summary>
        /// Endpoint name. Supports UTF-8 format
        /// </summary>
        public readonly string Name;
        /// <summary>
        /// Project name to which the endpoint belongs. Default is default
        /// </summary>
        public readonly string ProjectName;
        /// <summary>
        /// Security group ID for the endpoint IP address. The endpoint uses a default security group: For outbound endpoints, inbound traffic is denied; outbound traffic allows TCP port 53 and UDP port 53 to 0.0.0.0/0. For inbound endpoints, inbound traffic allows TCP port 53 and UDP port 53 to 0.0.0.0/0; outbound traffic is denied
        /// </summary>
        public readonly string SecurityGroupId;
        /// <summary>
        /// Endpoint status. Creating: creating. Running: running. Updating: updating. Error: abnormal operation
        /// </summary>
        public readonly string Status;
        /// <summary>
        /// Tag list
        /// </summary>
        public readonly ImmutableArray<Outputs.GetResolverEndpointTagResult> Tags;
        /// <summary>
        /// Update time
        /// </summary>
        public readonly string UpdatedTime;
        /// <summary>
        /// VPC ID where the endpoint is located
        /// </summary>
        public readonly string VpcId;
        /// <summary>
        /// Region of the VPC where the endpoint is located
        /// </summary>
        public readonly string VpcRegion;

        [OutputConstructor]
        private GetResolverEndpointResult(
            string createdTime,

            string direction,

            string endpointId,

            string endpointType,

            string id,

            ImmutableArray<Outputs.GetResolverEndpointIpConfigResult> ipConfigs,

            string name,

            string projectName,

            string securityGroupId,

            string status,

            ImmutableArray<Outputs.GetResolverEndpointTagResult> tags,

            string updatedTime,

            string vpcId,

            string vpcRegion)
        {
            CreatedTime = createdTime;
            Direction = direction;
            EndpointId = endpointId;
            EndpointType = endpointType;
            Id = id;
            IpConfigs = ipConfigs;
            Name = name;
            ProjectName = projectName;
            SecurityGroupId = securityGroupId;
            Status = status;
            Tags = tags;
            UpdatedTime = updatedTime;
            VpcId = vpcId;
            VpcRegion = vpcRegion;
        }
    }
}
