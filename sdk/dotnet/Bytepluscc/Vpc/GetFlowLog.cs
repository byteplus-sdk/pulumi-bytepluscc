// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Byteplus.Pulumi.Bytepluscc.Vpc
{
    public static class GetFlowLog
    {
        /// <summary>
        /// Data Source schema for Byteplus::VPC::FlowLog
        /// </summary>
        public static Task<GetFlowLogResult> InvokeAsync(GetFlowLogArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.InvokeAsync<GetFlowLogResult>("bytepluscc:vpc/getFlowLog:getFlowLog", args ?? new GetFlowLogArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::VPC::FlowLog
        /// </summary>
        public static Output<GetFlowLogResult> Invoke(GetFlowLogInvokeArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.Invoke<GetFlowLogResult>("bytepluscc:vpc/getFlowLog:getFlowLog", args ?? new GetFlowLogInvokeArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::VPC::FlowLog
        /// </summary>
        public static Output<GetFlowLogResult> Invoke(GetFlowLogInvokeArgs args, InvokeOutputOptions options)
            => global::Pulumi.Deployment.Instance.Invoke<GetFlowLogResult>("bytepluscc:vpc/getFlowLog:getFlowLog", args ?? new GetFlowLogInvokeArgs(), options.WithDefaults());
    }


    public sealed class GetFlowLogArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public string Id { get; set; } = null!;

        public GetFlowLogArgs()
        {
        }
        public static new GetFlowLogArgs Empty => new GetFlowLogArgs();
    }

    public sealed class GetFlowLogInvokeArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public Input<string> Id { get; set; } = null!;

        public GetFlowLogInvokeArgs()
        {
        }
        public static new GetFlowLogInvokeArgs Empty => new GetFlowLogInvokeArgs();
    }


    [OutputType]
    public sealed class GetFlowLogResult
    {
        /// <summary>
        /// 流日志的采样间隔。取值如下。1：1分钟。5：5分钟。10（默认值）：10分钟。
        /// </summary>
        public readonly int AggregationInterval;
        /// <summary>
        /// 流日志是否被锁定。Normal: 正常。FinancialLocked: 被锁定。
        /// </summary>
        public readonly string BusinessStatus;
        /// <summary>
        /// 流日志的创建时间。
        /// </summary>
        public readonly string CreatedAt;
        /// <summary>
        /// 流日志的描述信息。长度限制为0~ 255个字符。不填默认为空字符串。需要以字母、中文或数字开头。可包含英文逗号（,）、点号（.）、下划线（_）、空格（ ）、等号（=）、短横线（-）、中文逗号（，）、中文句号（。）。
        /// </summary>
        public readonly string Description;
        /// <summary>
        /// 是否自动配置索引。索引用于检索分析数据，流日志在检索分析之前，必须先配置索引。true：是。false（默认值）：否。
        /// </summary>
        public readonly bool EnableIndex;
        /// <summary>
        /// 流日志的ID。
        /// </summary>
        public readonly string FlowLogId;
        /// <summary>
        /// 流日志的名称。长度限制为1 ~ 128个字符。需要以字母、中文或数字开头，可包含点号（.）、下划线（_）和短横线（-）。不填默认为流日志实例的ID。
        /// </summary>
        public readonly string FlowLogName;
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        public readonly string Id;
        /// <summary>
        /// 流日志被锁定的原因。financial: 因欠费被锁定。unlock：欠费关停后充值恢复过程中。空值 : 没有被锁定。
        /// </summary>
        public readonly string LockReason;
        /// <summary>
        /// 流日志项目的ID。
        /// </summary>
        public readonly string LogProjectId;
        /// <summary>
        /// 流日志项目的名称。名称只能由小写字母、数字和连字符（-）组成。必须以小写字母或者数字开头和结尾。名称的长度范围为 3~63 个字符。
        /// </summary>
        public readonly string LogProjectName;
        /// <summary>
        /// 流日志主题的ID。
        /// </summary>
        public readonly string LogTopicId;
        /// <summary>
        /// 流日志项目的名称。名称只能由小写字母、数字和连字符（-）组成。必须以小写字母或者数字开头和结尾。名称的长度范围为 3~63 个字符。
        /// </summary>
        public readonly string LogTopicName;
        /// <summary>
        /// 流流日志所属项目的名称。
        /// </summary>
        public readonly string ProjectName;
        /// <summary>
        /// 要捕获流量的资源ID。
        /// </summary>
        public readonly string ResourceId;
        /// <summary>
        /// 要采集流量的资源类型，取值如下：vpc：私有网络。subnet：子网。eni：网卡。
        /// </summary>
        public readonly string ResourceType;
        /// <summary>
        /// 流日志的状态。取值如下：Active：启动状态。Pending：正在操作中。Inactive：未启动状态。Creating：正在创建中。Deleting：正在删除中。
        /// </summary>
        public readonly string Status;
        /// <summary>
        /// 流日志的标签信息。
        /// </summary>
        public readonly ImmutableArray<Outputs.GetFlowLogTagResult> Tags;
        /// <summary>
        /// 采集的流量类型。取值如下：All：全部流量。Allow：网络ACL和安全组允许的流量。Drop：网络ACL和安全组拒绝的流量。
        /// </summary>
        public readonly string TrafficType;
        /// <summary>
        /// 更新流日志的时间。
        /// </summary>
        public readonly string UpdatedAt;
        /// <summary>
        /// 流日志的VPC的ID。
        /// </summary>
        public readonly string VpcId;

        [OutputConstructor]
        private GetFlowLogResult(
            int aggregationInterval,

            string businessStatus,

            string createdAt,

            string description,

            bool enableIndex,

            string flowLogId,

            string flowLogName,

            string id,

            string lockReason,

            string logProjectId,

            string logProjectName,

            string logTopicId,

            string logTopicName,

            string projectName,

            string resourceId,

            string resourceType,

            string status,

            ImmutableArray<Outputs.GetFlowLogTagResult> tags,

            string trafficType,

            string updatedAt,

            string vpcId)
        {
            AggregationInterval = aggregationInterval;
            BusinessStatus = businessStatus;
            CreatedAt = createdAt;
            Description = description;
            EnableIndex = enableIndex;
            FlowLogId = flowLogId;
            FlowLogName = flowLogName;
            Id = id;
            LockReason = lockReason;
            LogProjectId = logProjectId;
            LogProjectName = logProjectName;
            LogTopicId = logTopicId;
            LogTopicName = logTopicName;
            ProjectName = projectName;
            ResourceId = resourceId;
            ResourceType = resourceType;
            Status = status;
            Tags = tags;
            TrafficType = trafficType;
            UpdatedAt = updatedAt;
            VpcId = vpcId;
        }
    }
}
