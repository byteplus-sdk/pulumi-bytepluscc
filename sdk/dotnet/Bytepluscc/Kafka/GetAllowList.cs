// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Byteplus.Pulumi.Bytepluscc.Kafka
{
    public static class GetAllowList
    {
        /// <summary>
        /// Data Source schema for Byteplus::Kafka::AllowList
        /// </summary>
        public static Task<GetAllowListResult> InvokeAsync(GetAllowListArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.InvokeAsync<GetAllowListResult>("bytepluscc:kafka/getAllowList:getAllowList", args ?? new GetAllowListArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::Kafka::AllowList
        /// </summary>
        public static Output<GetAllowListResult> Invoke(GetAllowListInvokeArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.Invoke<GetAllowListResult>("bytepluscc:kafka/getAllowList:getAllowList", args ?? new GetAllowListInvokeArgs(), options.WithDefaults());

        /// <summary>
        /// Data Source schema for Byteplus::Kafka::AllowList
        /// </summary>
        public static Output<GetAllowListResult> Invoke(GetAllowListInvokeArgs args, InvokeOutputOptions options)
            => global::Pulumi.Deployment.Instance.Invoke<GetAllowListResult>("bytepluscc:kafka/getAllowList:getAllowList", args ?? new GetAllowListInvokeArgs(), options.WithDefaults());
    }


    public sealed class GetAllowListArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public string Id { get; set; } = null!;

        public GetAllowListArgs()
        {
        }
        public static new GetAllowListArgs Empty => new GetAllowListArgs();
    }

    public sealed class GetAllowListInvokeArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        [Input("id", required: true)]
        public Input<string> Id { get; set; } = null!;

        public GetAllowListInvokeArgs()
        {
        }
        public static new GetAllowListInvokeArgs Empty => new GetAllowListInvokeArgs();
    }


    [OutputType]
    public sealed class GetAllowListResult
    {
        /// <summary>
        /// Allowlist rule list. Supports specifying IP addresses or IP ranges. Separate multiple IP addresses or ranges with commas (,). Each allowlist can contain up to 300 IP addresses or ranges
        /// </summary>
        public readonly string AllowList;
        /// <summary>
        /// Allowlist description. Maximum length: 200 characters
        /// </summary>
        public readonly string AllowListDesc;
        /// <summary>
        /// Allowlist ID
        /// </summary>
        public readonly string AllowListId;
        /// <summary>
        /// Number of rules specified in the allowlist
        /// </summary>
        public readonly int AllowListIpNum;
        /// <summary>
        /// Allowlist rule name. Length: 1–128 characters. Can only contain Chinese characters, letters, numbers, underscores, and hyphens (-). Cannot start with a number or hyphen (-). The allowlist name must be unique within the current region
        /// </summary>
        public readonly string AllowListName;
        /// <summary>
        /// Number of instances already bound to this allowlist. If the AllowList parameter is specified, this field is required to indicate the expected number of instances affected after rule modification. If the expected number does not match the actual number, the request fails
        /// </summary>
        public readonly int ApplyInstanceNum;
        /// <summary>
        /// Number of instances bound to the allowlist
        /// </summary>
        public readonly int AssociatedInstanceNum;
        /// <summary>
        /// Information about the instance bound to the allowlist
        /// </summary>
        public readonly ImmutableArray<Outputs.GetAllowListAssociatedInstanceResult> AssociatedInstances;
        /// <summary>
        /// Uniquely identifies the resource.
        /// </summary>
        public readonly string Id;

        [OutputConstructor]
        private GetAllowListResult(
            string allowList,

            string allowListDesc,

            string allowListId,

            int allowListIpNum,

            string allowListName,

            int applyInstanceNum,

            int associatedInstanceNum,

            ImmutableArray<Outputs.GetAllowListAssociatedInstanceResult> associatedInstances,

            string id)
        {
            AllowList = allowList;
            AllowListDesc = allowListDesc;
            AllowListId = allowListId;
            AllowListIpNum = allowListIpNum;
            AllowListName = allowListName;
            ApplyInstanceNum = applyInstanceNum;
            AssociatedInstanceNum = associatedInstanceNum;
            AssociatedInstances = associatedInstances;
            Id = id;
        }
    }
}
