// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "../utilities";

/**
 * Data Source schema for Byteplus::NatGateway::Snatentry
 */
export function getSnatentry(args: GetSnatentryArgs, opts?: pulumi.InvokeOptions): Promise<GetSnatentryResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("bytepluscc:natgateway/getSnatentry:getSnatentry", {
        "id": args.id,
    }, opts);
}

/**
 * A collection of arguments for invoking getSnatentry.
 */
export interface GetSnatentryArgs {
    /**
     * Uniquely identifies the resource.
     */
    id: string;
}

/**
 * A collection of values returned by getSnatentry.
 */
export interface GetSnatentryResult {
    /**
     * SNAT规则关联的公网IP的IP地址。返回单个公网IP的IP地址。 返回多个公网IP的IP地址。
     */
    readonly eipAddress: string;
    /**
     * SNAT规则关联的公网IP的ID。返回单个公网IP的ID。 返回多个公网IP的ID。
     */
    readonly eipId: string;
    /**
     * Uniquely identifies the resource.
     */
    readonly id: string;
    /**
     * SNAT规则所属NAT网关的ID。
     */
    readonly natGatewayId: string;
    /**
     * 私网NAT网关的中转IP的ID。
     */
    readonly natIpId: string;
    /**
     * SNAT规则的ID。
     */
    readonly snatEntryId: string;
    /**
     * SNAT规则的名称。
     */
    readonly snatEntryName: string;
    /**
     * SNAT规则对应的网段。
     */
    readonly sourceCidr: string;
    /**
     * SNAT规则的状态。Creating：创建中。Deleting：删除中。Available：可用。
     */
    readonly status: string;
    /**
     * SNAT规则关联子网的ID。
     */
    readonly subnetId: string;
}
/**
 * Data Source schema for Byteplus::NatGateway::Snatentry
 */
export function getSnatentryOutput(args: GetSnatentryOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetSnatentryResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("bytepluscc:natgateway/getSnatentry:getSnatentry", {
        "id": args.id,
    }, opts);
}

/**
 * A collection of arguments for invoking getSnatentry.
 */
export interface GetSnatentryOutputArgs {
    /**
     * Uniquely identifies the resource.
     */
    id: pulumi.Input<string>;
}
