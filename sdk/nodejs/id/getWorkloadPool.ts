// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

/**
 * Data Source schema for Byteplus::ID::WorkloadPool
 */
export function getWorkloadPool(args: GetWorkloadPoolArgs, opts?: pulumi.InvokeOptions): Promise<GetWorkloadPoolResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("bytepluscc:id/getWorkloadPool:getWorkloadPool", {
        "id": args.id,
    }, opts);
}

/**
 * A collection of arguments for invoking getWorkloadPool.
 */
export interface GetWorkloadPoolArgs {
    /**
     * Uniquely identifies the resource.
     */
    id: string;
}

/**
 * A collection of values returned by getWorkloadPool.
 */
export interface GetWorkloadPoolResult {
    /**
     * Workload pool creation time
     */
    readonly createdAt: string;
    /**
     * Workload pool description
     */
    readonly description: string;
    /**
     * OIDC discovery endpoint
     */
    readonly discoveryUrl: string;
    /**
     * Uniquely identifies the resource.
     */
    readonly id: string;
    /**
     * Project name
     */
    readonly projectName: string;
    /**
     * Resource tags. You can categorize instances by tags to facilitate instance search and resource aggregation.
     */
    readonly tags: outputs.id.GetWorkloadPoolTag[];
    /**
     * Number of associated credentials
     */
    readonly totalCredentials: number;
    /**
     * Number of workloads
     */
    readonly totalWorkloads: number;
    /**
     * Workload pool Trn
     */
    readonly trn: string;
    /**
     * Workload pool update time
     */
    readonly updatedAt: string;
    /**
     * Workload pool ID
     */
    readonly workloadPoolId: string;
    /**
     * Workload pool name
     */
    readonly workloadPoolName: string;
}
/**
 * Data Source schema for Byteplus::ID::WorkloadPool
 */
export function getWorkloadPoolOutput(args: GetWorkloadPoolOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetWorkloadPoolResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("bytepluscc:id/getWorkloadPool:getWorkloadPool", {
        "id": args.id,
    }, opts);
}

/**
 * A collection of arguments for invoking getWorkloadPool.
 */
export interface GetWorkloadPoolOutputArgs {
    /**
     * Uniquely identifies the resource.
     */
    id: pulumi.Input<string>;
}
