// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "../utilities";

/**
 * Data Source schema for Byteplus::CloudIdentity::PermissionSetProvisioning
 */
export function getPermissionSetProvisioning(args: GetPermissionSetProvisioningArgs, opts?: pulumi.InvokeOptions): Promise<GetPermissionSetProvisioningResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invoke("bytepluscc:cloudidentity/getPermissionSetProvisioning:getPermissionSetProvisioning", {
        "id": args.id,
    }, opts);
}

/**
 * A collection of arguments for invoking getPermissionSetProvisioning.
 */
export interface GetPermissionSetProvisioningArgs {
    /**
     * Uniquely identifies the resource.
     */
    id: string;
}

/**
 * A collection of values returned by getPermissionSetProvisioning.
 */
export interface GetPermissionSetProvisioningResult {
    /**
     * Creation time
     */
    readonly createdTime: string;
    /**
     * Uniquely identifies the resource.
     */
    readonly id: string;
    /**
     * Permission set ID
     */
    readonly permissionSetId: string;
    /**
     * Permission set name
     */
    readonly permissionSetName: string;
    /**
     * Permission set status Running: In progress, Reprovision Required: Redeployment required, Provision Failed: Deployment failed, Provisioned: Deployed
     */
    readonly provisioningStatus: string;
    /**
     * Authorized account ID
     */
    readonly targetId: string;
    /**
     * Account name
     */
    readonly targetName: string;
    /**
     * Update time
     */
    readonly updatedTime: string;
}
/**
 * Data Source schema for Byteplus::CloudIdentity::PermissionSetProvisioning
 */
export function getPermissionSetProvisioningOutput(args: GetPermissionSetProvisioningOutputArgs, opts?: pulumi.InvokeOutputOptions): pulumi.Output<GetPermissionSetProvisioningResult> {
    opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts || {});
    return pulumi.runtime.invokeOutput("bytepluscc:cloudidentity/getPermissionSetProvisioning:getPermissionSetProvisioning", {
        "id": args.id,
    }, opts);
}

/**
 * A collection of arguments for invoking getPermissionSetProvisioning.
 */
export interface GetPermissionSetProvisioningOutputArgs {
    /**
     * Uniquely identifies the resource.
     */
    id: pulumi.Input<string>;
}
